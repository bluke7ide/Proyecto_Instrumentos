...
Integrar lambda techo, que es constlam


P es Bn
```{python}
def exact_caplet(L, constlam, N, T, K, delta):
    ...
    for n in range(1, N + 1):
        for j in range(n):
            P[n] *= 1 / (1 + delta * L[j, 0]) 
    for n in range(1, N+1):
        caplet[n-1] = cblack(constlam, K[n], L[n, 0], P[n], T[n], delta)
...
def simulate_caplet(caplet, L, constlam, N, T, K, delta, h, niter):
    ...
    for i in range(1, niter + 1):
        simulate_terminal(L, constlam, N, delta, h, -i)

        for n in range(1, N):
            den = 1
            for j in range(n + 1, N + 1):
                den *= 1 / (1 + delta * L[j, n + 1])
...
L = np.random.rand(N+1, 1) * 0.1  



caplet = exact_caplet(L, constlam, N, T, K, delta)

# Crear o buscar L, T, L
# caplet es un retorno, sacar
# niter se corre con 10000
```
...
def european_swaption(alpha, L, constlam, N, T, K, delta, h, niter):
    ...
    avg = 0  # Promedio de los resultados
    for k in range(1, int(niter) + 1):
        simulate_terminal(L, constlam, N, delta, h, -k)  # Simula tasas futuras

        den = 1
        for j in range(alpha + 1, N + 1):
            den *= 1 / (1 + delta * L[j, alpha + 1])

        swaption = 0
        for i in range(alpha + 1, N + 2):
            Bi = 1
            for j in range(alpha, i):
                Bi *= 1 / (1 + delta * L[j, alpha])
            swaption += Bi * delta * (L[i - 1, alpha] - K)

        ans = num * max(0,swaption) / den
        avg += ans / niter
